closeup_pictures_coordinates_shifting <- function(input_folder,
  ...
  # Filter out files containing the word 'zoom' in their names
  wide_files <- image_files[!grepl("zoom", basename(image_files), ignore.case = TRUE)]
  ...
  # Copy files to afterppk folder before updating
  output_wide_file <- file.path(output_folder, basename(wide_file))
  output_zoom_file <- file.path(output_folder, basename(zoom_file))

  file.copy(wide_file, output_wide_file, overwrite = TRUE)
  file.copy(zoom_file, output_zoom_file, overwrite = TRUE)
  ...
  tryCatch({
    ...
    path = c(output_wide_file, output_zoom_file)
    )
    success_count <- success_count + 1
  }, error = function(e) {
    warning(paste("Failed to update EXIF metadata for", basename(output_wide_file), " and ", basename(output_zoom_file), ":", e$message))
    error_count <- error_count + 1
  })
  }
...
# input_folder <- "/path/to/input/folder"                      # path to folder containing close-up (Wide and zoom) pictures
