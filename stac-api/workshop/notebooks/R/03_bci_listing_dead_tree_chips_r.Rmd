---
title: "List COGs for collection `2024_bci`"
...
This notebook queries the Kanopia STAC API, lists all COG assets matching the
`2024_bci` collection and the `*bciwhole*rgb.cog.tif` pattern, then extracts a
512x512 pixel PNG chip centred on a target tree for each flight date.
